...
class CommonParser:
    def columnParser(self,logsource,lines,pattern='\[|\]|[A-Za-z]+:\s|USER|REMOTE|ms|START|STATUS|X-Forwarded-For(.)|(\d\d){2}\d\d\d\d', replacement=''):
    ...
    def writeDictCSV(self,directory,filename,overwrite=False,sort=True,**dict):
        ...
        fp.close()
    def releaseParser(self,directory,releaseId):
        if releaseId !=0:
    ...
    def apiParser(self,directory,fileName,api):
        if api !="":
            searchString=api
